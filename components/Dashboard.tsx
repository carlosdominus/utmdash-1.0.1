...
const Dashboard: React.FC<DashboardProps> = ({ data }) => {
  ...
  const uniqueValuesMap = useMemo(() => {
    ...
    categoricalFilterCols.forEach(col => {
      const vals = Array.from(new Set<string>(data.rows.map(r => String(r[col] ?? 'N/A')).filter(v => v !== '')));
      map[col] = vals.sort();
    });
    return map;
  ...
  const filteredRows = useMemo(() => {
    ...
    return rowsWithIndex.filter(row => {
      ...
      const matchesFilters = Object.entries(filters).every(([col, vals]) => {
        ...
        const rowVal = String(row[col] ?? 'N/A');
  ...
  // Agrupamento para o UTM DASH (Produto + Campanha + Termo)
  const groupedPerformance = useMemo(() => {
    const groups: Record<string, any> = {};
    filteredRows.forEach(row => {
      const p = String(row[colProduto || ''] || 'N/A');
      const c = String(row[colCampaign || ''] || 'N/A');
      const t = String(row[colTerm || ''] || 'N/A');
      const key = `${p}|${c}|${t}`;
      ...
      // Simples comparação de strings para datas BR (não ideal mas funciona para o label)
      if (rowDateStr < groups[key].minDate) groups[key].minDate = rowDateStr;
      if (rowDateStr > groups[key].maxDate) groups[key].maxDate = rowDateStr;
    });

    return Object.values(groups).sort((a, b) => b.vendas - a.vendas);
  }, [filteredRows, colProduto, colCampaign, colTerm, colData, colFaturamento]);
  ...
  const getTop5 = (colName: string | undefined) => {
    ...
    filteredRows.forEach(row => {
      const val = String(row[colName] || 'N/A').trim();
  ...
  const top5Terms = useMemo(() => getTop5(colTerm), [filteredRows, colTerm]);
        ...
        <div className="bg-white rounded-[32px] border border-slate-200 shadow-xl overflow-hidden animate-in fade-in zoom-in duration-300 w-full">
          <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex flex-col md:flex-row justify-between items-center gap-4">
            <div className="flex flex-col">
              ...
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">AGRUPADO POR PRODUTO + CAMPANHA + TERMO</p>
            </div>
                  ...
                  <HeaderCell label="PERÍODO" width="140px" active={activeHeaderFilter === 'period'} onClick={() => setActiveHeaderFilter(activeHeaderFilter === 'period' ? null : 'period')} hasFilter={datePreset !== 'all'}>
                    <div className="p-4 space-y-4">
                      <div className="grid grid-cols-2 gap-2">
                        {['all', 'today', '7days', '15days', '30days', 'custom'].map(p => (
                          <button 
                            key={p} 
                            onClick={() => { setDatePreset(p as DatePreset); if(p !== 'custom') setActiveHeaderFilter(null); }}
                            className={`px-3 py-2 rounded-lg text-[10px] font-black uppercase transition-all ${datePreset === p ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                          >
                            {p === 'all' ? 'TUDO' : p === '7days' ? '7 DIAS' : p === '15days' ? '15 DIAS' : p === '30days' ? '30 DIAS' : p.toUpperCase()}
                          </button>
                        ))}
                      </div>
                      {datePreset === 'custom' && (
                        <div className="space-y-2 animate-in slide-in-from-top-2 duration-200">
                          <input type="date" className="w-full p-2 bg-slate-50 border rounded-lg text-xs" value={customStartDate} onChange={e => setCustomStartDate(e.target.value)} />
                          <input type="date" className="w-full p-2 bg-slate-50 border rounded-lg text-xs" value={customEndDate} onChange={e => setCustomEndDate(e.target.value)} />
                        </div>
                      )}
                    </div>
                  </HeaderCell>

                  <HeaderCell label="PRODUTO" active={activeHeaderFilter === 'produto'} onClick={() => setActiveHeaderFilter(activeHeaderFilter === 'produto' ? null : 'produto')} hasFilter={filters[colProduto || '']?.length > 0}>
                    <FilterDropdownContent col={colProduto || ''} values={uniqueValuesMap[colProduto || '']} filters={filters} toggle={toggleFilter} />
                  </HeaderCell>

                  <HeaderCell label="CAMPANHA" active={activeHeaderFilter === 'campanha'} onClick={() => setActiveHeaderFilter(activeHeaderFilter === 'campanha' ? null : 'campanha')} hasFilter={filters[colCampaign || '']?.length > 0}>
                    <FilterDropdownContent col={colCampaign || ''} values={uniqueValuesMap[colCampaign || '']} filters={filters} toggle={toggleFilter} />
                  </HeaderCell>

                  <HeaderCell label="UTM TERM" active={activeHeaderFilter === 'termo'} onClick={() => setActiveHeaderFilter(activeHeaderFilter === 'termo' ? null : 'termo')} hasFilter={filters[colTerm || '']?.length > 0}>
                    <FilterDropdownContent col={colTerm || ''} values={uniqueValuesMap[colTerm || '']} filters={filters} toggle={toggleFilter} />
                  </HeaderCell>
                ...
                {groupedPerformance.map((group) => {
                  ...
                  return (
                    <tr key={group.key} className={`transition-colors ${hasProfit ? 'bg-emerald-50/30' : isLoss ? 'bg-rose-50/30' : 'hover:bg-slate-50'}`}>
                      ...
                      <td className="px-4 py-4 font-black text-slate-700">{group.produto}</td>
                      <td className="px-4 py-4 font-medium text-slate-500 max-w-[200px] truncate" title={group.campanha}>{group.campanha}</td>
                      <td className="px-4 py-4 font-medium text-slate-500">{group.termo}</td>
                      ...
                      <td className="px-4 py-4">
                        <div className="flex items-center px-3 py-2 bg-white border border-indigo-100 rounded-xl focus-within:ring-2 focus-within:ring-indigo-500 shadow-sm">
                          ...
                          <input 
                            type="number" 
                            className="bg-transparent border-none outline-none w-16 text-[11px] font-black text-slate-800 p-0"
                            value={groupInvestments[group.key] || ''}
                            onChange={(e) => setGroupInvestments(prev => ({ ...prev, [group.key]: Number(e.target.value) }))}
                            placeholder="0,00"
                          />
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-4 text-right pr-6">
                        {investment === 0 ? (
                          ...
                          <span className={`inline-block px-4 py-1.5 rounded-full font-black text-[10px] uppercase tracking-wider shadow-sm ${roi >= 1 ? 'bg-emerald-500 text-white' : 'bg-rose-500 text-white'}`}>
                            {roi.toFixed(2)}x ROI
                          </span>
                        )}
                      </td>
        ...
        <div className="space-y-8 animate-in fade-in duration-500 w-full">
          ...
          <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm w-full">
            <div className="flex items-center space-x-2 mb-10">
              ...
              <h4 className="text-xs font-black text-slate-800 uppercase tracking-widest">Evolução de Vendas</h4>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full">
            ...
            <DonutChartCard title="TERMOS (TOP 5)" data={top5Terms} icon={<Layers className="w-4 h-4" />} />
          ...
          <div className="overflow-x-auto max-h-[750px] scrollbar-thin">
            <table className="w-full text-left text-[11px] border-collapse relative">
              <thead className="sticky top-0 z-20 bg-slate-50 shadow-sm">
                <tr className="border-b border-slate-200">
                  {data.headers.map(h => (<th key={h} className="px-6 py-4 font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">{h}</th>))}
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
...
// Componentes UTM DASH Específicos
const HeaderCell = ({ label, children, active, onClick, hasFilter, width }: any) => {
  const ref = useRef<HTMLTableHeaderCellElement>(null);
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      if (active && ref.current && !ref.current.contains(e.target as Node)) onClick();
    };
  ...
  return (
    <th ref={ref} className="px-4 py-4 relative group" style={{ width }}>
      <button 
        onClick={onClick}
        className={`flex items-center font-black uppercase tracking-widest transition-all hover:text-indigo-600 ${hasFilter ? 'text-indigo-600' : 'text-slate-400'}`}
      >
        {label}
        <ChevronDown className={`w-3 h-3 ml-1.5 transition-transform ${active ? 'rotate-180 text-indigo-600' : 'opacity-40 group-hover:opacity-100'}`} />
      </button>
      {active && (
        <div className="absolute top-full left-0 mt-2 bg-white border border-slate-200 shadow-2xl rounded-2xl z-50 animate-in fade-in zoom-in duration-150 origin-top-left min-w-[280px]">
          {children}
        </div>
      )}
    </th>
    ...
    <div className="p-4 space-y-3">
      <div className="relative">
        ...
        <input 
          ...
          className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-100 rounded-xl text-[11px] font-bold outline-none focus:ring-2 focus:ring-indigo-500"
          value={search}
      ...
      <div className="max-h-60 overflow-y-auto space-y-1 scrollbar-thin">
        {values.filter((v: any) => String(v).toLowerCase().includes(search.toLowerCase())).map((val: any) => (
          <button 
            key={val} 
            onClick={() => toggle(col, val)}
            className={`w-full text-left px-3 py-2 rounded-lg text-[10px] font-bold flex items-center justify-between transition-colors ${filters[col]?.includes(val) ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600 hover:bg-slate-50'}`}
          >
      ...
      <ResponsiveContainer width="100%" height="100%">
        <PieChart>
          <Pie data={data} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
            {data.map((_: any, index: number) => (<Cell key={`cell-${index}`} fill={PIE_COLORS[index % PIE_COLORS.length]} strokeWidth={0} />))}
          </Pie>
    ...
    <div className="flex flex-wrap gap-x-4 gap-y-2 justify-start">
      {data.map((entry: any, index: number) => (
        <div key={entry.name} className="flex items-center space-x-2">
          ...
          <span className="text-[9px] font-bold text-slate-500 truncate max-w-[120px]">{entry.name}</span>
        </div>
      ))}
    </div>
  </div>
...
const FilterColumn = ({ col, uniqueValues, filters, toggleFilter }: any) => {
  ...
  return (
    <div className="space-y-3 flex flex-col">
      ...
      <div className="relative">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-300" />
        <input type="text" placeholder={`Buscar em ${col.toLowerCase()}...`} className="w-full pl-9 pr-4 py-2.5 bg-slate-50 border border-slate-100 rounded-xl text-[10px] font-bold focus:ring-2 focus:ring-indigo-500 outline-none transition-all" value={search} onChange={(e) => setSearch(e.target.value)} />
      </div>
      <div className="flex-1 max-h-52 overflow-y-auto border border-slate-100 rounded-2xl p-2 bg-slate-50 space-y-1 scrollbar-thin shadow-inner">
        {(uniqueValues || []).filter((o: any) => String(o).toLowerCase().includes(search.toLowerCase())).map((val: any) => (
          <button key={val} onClick={() => toggleFilter(col, val)} className={`w-full flex items-center justify-between p-2.5 rounded-xl text-[10px] font-bold transition-all text-left ${filters[col]?.includes(val) ? 'bg-indigo-600 text-white shadow-md' : 'bg-white text-slate-600 hover:bg-slate-100'}`}>
            <span className="truncate flex-1">{val}</span>
...
const StatCard = ({ title, value, icon, color, tag }: any) => {
  ...
  return (
    <div className="bg-white p-5 rounded-[28px] border border-slate-100 shadow-sm relative group overflow-hidden">
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-3"><div className={`p-1.5 ${styles.lightBg} ${styles.text} rounded-lg`}>{icon}</div><span className={`text-[9px] font-black uppercase ${styles.lightBg} ${styles.text} px-2 py-0.5 rounded-full`}>{tag}</span></div>
...
const TabButton = ({ active, onClick, label, icon }: any) => (
  <button onClick={onClick} className={`flex items-center px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${active ? 'bg-white text-indigo-600 shadow-md scale-105' : 'text-slate-500 hover:text-slate-800'}`}>
    {icon} {label}
  </button>
);
